...
class CliTraefikTest < CliTestCase
  test "boot" do
    run_command("boot").tap do |output|
      ...
      assert_match "docker run --name traefik --detach --restart unless-stopped --publish 80:80 --volume /var/run/docker.sock:/var/run/docker.sock --log-opt max-size=\"10m\" #{Pnmx::Commands::Traefik::DEFAULT_IMAGE} --providers.docker --log.level=\"DEBUG\"", output
    end
  end

  test "reboot" do
    Pnmx::Cli::Traefik.any_instance.expects(:stop)
    Pnmx::Cli::Traefik.any_instance.expects(:remove_container)
    Pnmx::Cli::Traefik.any_instance.expects(:boot)
  ...
  test "restart" do
    Pnmx::Cli::Traefik.any_instance.expects(:stop)
    Pnmx::Cli::Traefik.any_instance.expects(:start)
  ...
  test "remove" do
    Pnmx::Cli::Traefik.any_instance.expects(:stop)
    Pnmx::Cli::Traefik.any_instance.expects(:remove_container)
    Pnmx::Cli::Traefik.any_instance.expects(:remove_image)
  ...
  private
    def run_command(*command)
      stdouted { Pnmx::Cli::Traefik.start([*command, "-c", "test/fixtures/deploy_with_accessories.yml"]) }
    end
end
