...
class CliPruneTest < CliTestCase
  test "all" do
    Pnmx::Cli::Prune.any_instance.expects(:containers)
    Pnmx::Cli::Prune.any_instance.expects(:images)
  ...
  private
    def run_command(*command)
      stdouted { Pnmx::Cli::Prune.start([*command, "-c", "test/fixtures/deploy_with_accessories.yml"]) }
    end
end
