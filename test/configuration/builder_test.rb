...
class ConfigurationBuilderTest < ActiveSupport::TestCase
  setup do
    ...
    @config_with_builder_option = Pnmx::Configuration.new(@deploy_with_builder_option)
  end
  ...
  test "setting registry cache with image" do
    @deploy_with_builder_option[:builder] = { "cache" => { "type" => "registry", "image" => "pnmx", "options" => "mode=max" } }

    assert_equal "type=registry,ref=/pnmx", @config_with_builder_option.builder.cache_from
    assert_equal "type=registry,mode=max,ref=/pnmx", @config_with_builder_option.builder.cache_to
  end
