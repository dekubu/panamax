...
class Pnmx::Commander
  ...
  def app(role: nil)
    Pnmx::Commands::App.new(config, role: role)
  end

  def accessory(name)
    Pnmx::Commands::Accessory.new(config, name: name)
  end

  def auditor(**details)
    Pnmx::Commands::Auditor.new(config, **details)
  end

  def builder
    @builder ||= Pnmx::Commands::Builder.new(config)
  end

  def docker
    @docker ||= Pnmx::Commands::Docker.new(config)
  end

  def healthcheck
    @healthcheck ||= Pnmx::Commands::Healthcheck.new(config)
  end

  def hook
    @hook ||= Pnmx::Commands::Hook.new(config)
  end

  def lock
    @lock ||= Pnmx::Commands::Lock.new(config)
  end

  def prune
    @prune ||= Pnmx::Commands::Prune.new(config)
  end

  def registry
    @registry ||= Pnmx::Commands::Registry.new(config)
  end

  def traefik
    @traefik ||= Pnmx::Commands::Traefik.new(config)
  end
