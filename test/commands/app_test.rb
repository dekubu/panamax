...
class CommandsAppTest < ActiveSupport::TestCase
  ...
  test "run with custom options" do
    ...
    assert_equal \
      "docker run --detach --restart unless-stopped --name app-jobs-999 -e PNMX_CONTAINER_NAME=\"app-jobs-999\" -e RAILS_MASTER_KEY=\"456\" --log-opt max-size=\"10m\" --label service=\"app\" --label role=\"jobs\" --mount \"somewhere\" --cap-add dhh/app:999 bin/jobs",
  ...
  private
    def new_command(role: "web")
      Pnmx::Commands::App.new(Pnmx::Configuration.new(@config, destination: @destination, version: "999"), role: role)
    end
end
