...
class ConfigurationTest < ActiveSupport::TestCase
  setup do
    ...
    @config = Pnmx::Configuration.new(@deploy)

    @deploy_with_roles = @deploy.dup.merge({
      servers: { "web" => [ "1.1.1.1", "1.1.1.2" ], "workers" => { "hosts" => [ "1.1.1.1", "1.1.1.3" ] } } })

    @config_with_roles = Pnmx::Configuration.new(@deploy_with_roles)
  end
  ...
  %i[ service image registry ].each do |key|
    test "#{key} config required" do
      assert_raise(ArgumentError) do
        Pnmx::Configuration.new @deploy.tap { _1.delete key }
      end
    end
  end
  ...
  %w[ username password ].each do |key|
    test "registry #{key} required" do
      assert_raise(ArgumentError) do
        Pnmx::Configuration.new @deploy.tap { _1[:registry].delete key }
      end
    end
  end
  ...
  test "traefik hosts" do
    ...
    config = Pnmx::Configuration.new(@deploy_with_roles)
  ...
  test "repository" do
    ...
    config = Pnmx::Configuration.new(@deploy.tap { |c| c[:registry].merge!({ "server" => "ghcr.io" }) })
  ...
  test "env args with clear and secrets" do
    ...
    config = Pnmx::Configuration.new(@deploy.tap { |c| c.merge!({
      env: { "clear" => { "PORT" => "3000" }, "secret" => [ "PASSWORD" ] }
    }) })

    assert_equal [ "-e", "PASSWORD=\"secret123\"", "-e", "PORT=\"3000\"" ], Pnmx::Utils.unredacted(config.env_args)
    assert_equal [ "-e", "PASSWORD=[REDACTED]", "-e", "PORT=\"3000\"" ], Pnmx::Utils.redacted(config.env_args)
  ensure
    ENV["PASSWORD"] = nil
  end
  ...
  test "env args with only secrets" do
    ...
    assert_equal [ "-e", "PASSWORD=\"secret123\"" ], Pnmx::Utils.unredacted(config.env_args)
    assert_equal [ "-e", "PASSWORD=[REDACTED]" ], Pnmx::Utils.redacted(config.env_args)
  ensure
    ENV["PASSWORD"] = nil
  end
  ...
  test "hosts required for all roles" do
    ...
    assert_raises(ArgumentError) do
      Pnmx::Configuration.new @deploy.merge(servers: [])
    end

    # Empty server list
    assert_raises(ArgumentError) do
      Pnmx::Configuration.new @deploy.merge(servers: { "web" => [] })
    end
    ...
    # Empty hosts list
    assert_raises(ArgumentError) do
      Pnmx::Configuration.new @deploy.merge(servers: { "web" => { "hosts" => [] } })
    end

    # Nil hosts
    assert_raises(ArgumentError) do
      Pnmx::Configuration.new @deploy.merge(servers: { "web" => { "hosts" => nil } })
    end

    # One role with hosts, one without
    assert_raises(ArgumentError) do
      Pnmx::Configuration.new @deploy.merge(servers: { "web" => %w[ web ], "workers" => { "hosts" => %w[ ] } })
    end
  end
  ...
  test "ssh options" do
    ...
    config = Pnmx::Configuration.new(@deploy.tap { |c| c.merge!(ssh: { "user" => "app" }) })
    assert_equal "app", @config.ssh_options[:user]
  end

  test "ssh options with proxy host" do
    config = Pnmx::Configuration.new(@deploy.tap { |c| c.merge!(ssh: { "proxy" => "1.2.3.4" }) })
    assert_equal "root@1.2.3.4", @config.ssh_options[:proxy].jump_proxies
  end

  test "ssh options with proxy host and user" do
    config = Pnmx::Configuration.new(@deploy.tap { |c| c.merge!(ssh: { "proxy" => "app@1.2.3.4" }) })
  ...
  test "logging args with configured options" do
    config = Pnmx::Configuration.new(@deploy.tap { |c| c.merge!(logging: { "options" => { "max-size" => "100m", "max-file" => 5 } }) })
  ...
  test "logging args with configured driver and options" do
    config = Pnmx::Configuration.new(@deploy.tap { |c| c.merge!(logging: { "driver" => "local", "options" => { "max-size" => "100m", "max-file" => 5 } }) })
    assert_equal ["--log-driver", "\"local\"", "--log-opt", "max-size=\"100m\"", "--log-opt", "max-file=\"5\""], @config.logging_args
  end

  test "erb evaluation of yml config" do
    config = Pnmx::Configuration.create_from config_file: Pathname.new(File.expand_path("fixtures/deploy.erb.yml", __dir__))
  ...
  test "destination yml config merge" do
    ...
    config = Pnmx::Configuration.create_from config_file: dest_config_file, destination: "world"
    assert_equal "1.1.1.1", config.all_hosts.first

    config = Pnmx::Configuration.create_from config_file: dest_config_file, destination: "mars"
  ...
  test "destination yml config file missing" do
    ...
    assert_raises(RuntimeError) do
      config = Pnmx::Configuration.create_from config_file: dest_config_file, destination: "missing"
    end
  end
  ...
  test "min version is lower" do
    config = Pnmx::Configuration.new(@deploy.tap { |c| c.merge!(minimum_version: "0.0.1") })
    assert_equal "0.0.1", config.minimum_version
  end

  test "min version is equal" do
    config = Pnmx::Configuration.new(@deploy.tap { |c| c.merge!(minimum_version: Pnmx::VERSION) })
    assert_equal Pnmx::VERSION, config.minimum_version
  end

  test "min version is higher" do
    assert_raises(ArgumentError) do
      Pnmx::Configuration.new(@deploy.tap { |c| c.merge!(minimum_version: "10000.0.0") })
    end
  end
end
