...
PNMX deploys web apps anywhere from bare metal to cloud VMs using Docker with zero downtime. It uses the dynamic reverse-proxy Traefik to hold requests while the new application container is started and the old one is stopped. It works seamlessly across multiple hosts, using LXDKit to execute commands. It was built for Rails applications, but works with any type of web app that can be containerized with Docker.
...
registry:
  ...
  password:
    - PNMX_REGISTRY_PASSWORD
env:
  secret:
    - RAILS_MASTER_KEY
```

Then edit your `.env` file to add your registry password as `PNMX_REGISTRY_PASSWORD` (and your `RAILS_MASTER_KEY` for production with a Rails app).
...
Voila! All the servers are now serving the app on port 80. If you're just running a single server, you're ready to go. If you're running multiple servers, you need to put a load balancer in front of them. For subsequent deploys, or if your servers already have Docker and curl installed, you can just run `pnmx deploy`.
...
PNMX uses [dotenv](https://github.com/bkeepers/dotenv) to automatically load environment variables set in the `.env` file present in the application root. This file can be used to set variables like `PNMX_REGISTRY_PASSWORD` or database passwords. But for this reason you must ensure that .env files are not checked into Git or included in your Dockerfile! The format is just key-value like:

```bash
PNMX_REGISTRY_PASSWORD=pw
...
```erb
<% if (session_token = `op signin --account my-one-password-account --raw`.strip) != "" %># Generated by pnmx envify
GITHUB_TOKEN=<%= `gh config get -h github.com oauth_token`.strip %>
PNMX_REGISTRY_PASSWORD=<%= `op read "op://Vault/Docker Hub/password" -n --session  #{session_token}` %>
RAILS_MASTER_KEY=<%= `op read "op://Vault/My App/RAILS_MASTER_SECRET" -n --session #{session_token}` %>
MYSQL_ROOT_PASSWORD=<%= `op read "op://Vault/My App/MYSQL_ROOT_PASSWORD" -n --session #{session_token}` %>
<% else raise ArgumentError, "Session token missing" end %>
```

This template can safely be checked into git. Then everyone deploying the app can run `pnmx envify` when they setup the app for the first time or passwords change to get the correct `.env` file.

If you need separate env variables for different destinations, you can set them with `.env.destination.erb` for the template, which will generate `.env.staging` when run with `pnmx envify -d staging`.
...
```erb
<% if (session_token=`bw unlock --raw`.strip) != "" %># Generated by pnmx envify
SOME_SECRET=<%= `bw get notes 123123123-1232-4224-222f-234234234234 --session #{session_token}` %>
<% else raise ArgumentError, "session_token token missing" end %>
```

Then everyone deploying the app can run `pnmx envify` and pnmx will generate `.env`
...
`PNMX_CONTAINER_NAME` : this contains the current container name and version
...
Now run `pnmx accessory start mysql` to start the MySQL server on 1.1.1.3. See `pnmx accessory` for all the commands possible.
...
pnmx app exec --primary 'cat .ruby-version'
App Host: 192.168.0.1
3.1.3

# Runs Rails command on all servers
pnmx app exec 'bin/rails about'
App Host: 192.168.0.1
...
pnmx app exec -p 'bin/rails runner "puts Rails.application.config.time_zone"'
UTC
```
...
You can also see just info for app containers with `pnmx app details` or just for Traefik with `pnmx traefik details`.

### Running rollback to fix a bad deploy

If you've discovered a bad deploy, you can quickly rollback by reactivating the old, paused container image. You can see what old containers are available for rollback by running `pnmx app containers`. It'll give you a presentation similar to `pnmx app details`, but include all the old containers as well. Showing something like this:
...
From the example above, we can see that `e5d9d7c2b898289dfbc5f7f1334140d984eedae4` was the last version, so it's available as a rollback target. We can perform this rollback by running `pnmx rollback e5d9d7c2b898289dfbc5f7f1334140d984eedae4`. That'll stop `6ef8a6a84c525b123c5245345a8483f86d05a123` and then start `e5d9d7c2b898289dfbc5f7f1334140d984eedae4`. Because the old container is still available, this is very quick. Nothing to download from the registry.

Note that by default old containers are pruned after 3 days when you run `pnmx deploy`.

### Running removal to clean up servers

If you wish to remove the entire application, including Traefik, containers, images, and registry session, you can run `pnmx remove`. This will leave the servers clean.

## Locking

Commands that are unsafe to run concurrently will take a deploy lock while they run. The lock is the `pnmx_lock` directory on the primary server.
...
```
pnmx lock acquire -m "Doing maintenance"
```
...
Hooks should be stored in the .pnmx/hooks folder. Running pnmx init will build that folder and add some sample scripts.
...
`PNMX_*` environment variables are available to the hooks command for
fine-grained audit reporting, e.g. for triggering deployment reports or
firing a JSON webhook. These variables include:
- `PNMX_RECORDED_AT` - UTC timestamp in ISO 8601 format, e.g. `2023-04-14T17:07:31Z`
- `PNMX_PERFORMER` - the local user performing the command (from `whoami`)
- `PNMX_SERVICE_VERSION` - an abbreviated service and version for use in messages, e.g. app@150b24f
- `PNMX_VERSION` - the full version being deployed
- `PNMX_HOSTS` - a comma-separated list of the hosts targeted by the command
- `PNMX_COMMAND` - The command we are running
- `PNMX_SUBCOMMAND` - optional: The subcommand we are running
- `PNMX_DESTINATION` - optional: destination, e.g. "staging"
- `PNMX_ROLE` - optional: role targeted, e.g. "web"
...
This hook is also passed a `PNMX_RUNTIME` env variable set to the total seconds the deploy took.
...
```bash
#!/usr/bin/env bash
curl -q -d content="[My App] ${PNMX_PERFORMER} Rolled back to version ${PNMX_VERSION}" https://3.basecamp.com/XXXXX/integrations/XXXXX/buckets/XXXXX/chats/XXXXX/lines
```
