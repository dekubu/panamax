...
class CommandsTraefikTest < ActiveSupport::TestCase
  ...
  test "run without configuration" do
    ...
    assert_equal \
      "docker run --name traefik --detach --restart unless-stopped --publish 80:80 --volume /var/run/docker.sock:/var/run/docker.sock --log-opt max-size=\"10m\" #{Pnmx::Commands::Traefik::DEFAULT_IMAGE} --providers.docker --log.level=\"DEBUG\"",
      new_command.run.join(" ")
  end
  ...
  private
    def new_command
      Pnmx::Commands::Traefik.new(Pnmx::Configuration.new(@config, version: "123"))
    end
end
