...
class CliBuildTest < CliTestCase
  test "deliver" do
    Pnmx::Cli::Build.any_instance.expects(:push)
    Pnmx::Cli::Build.any_instance.expects(:pull)
  ...
  test "push" do
    ...
    run_command("push").tap do |output|
      ...
      assert_match /docker buildx build --push --platform linux\/amd64,linux\/arm64 --builder pnmx-app-multiarch -t dhh\/app:999 -t dhh\/app:latest --label service="app" --file Dockerfile \. as .*@localhost/, output
    end
  end
  ...
  test "push pre-build hook failure" do
    ...
    assert_raises(Pnmx::Cli::HookError) { run_command("push") }
  ...
  test "create" do
    run_command("create").tap do |output|
      assert_match /docker buildx create --use --name pnmx-app-multiarch/, output
    end
  end
  ...
  test "remove" do
    run_command("remove").tap do |output|
      assert_match /docker buildx rm pnmx-app-multiarch/, output
    end
  end
  ...
  private
    def run_command(*command)
      stdouted { Pnmx::Cli::Build.start([*command, "-c", "test/fixtures/deploy_with_accessories.yml"]) }
    end
