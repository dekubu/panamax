...
class ConfigurationRoleTest < ActiveSupport::TestCase
  ...
  test "default traefik label on non-web role" do
    config = Pnmx::Configuration.new(@deploy_with_roles.tap { |c|
  ...
  test "env secret overwritten by role" do
    ...
    @config_with_roles.role(:workers).env_args.tap do |env_args|
      assert_equal ["-e", "REDIS_PASSWORD=\"secret456\"", "-e", "DB_PASSWORD=\"secret&\\\"123\"", "-e", "REDIS_URL=\"redis://a/b\"", "-e", "WEB_CONCURRENCY=\"4\""], Pnmx::Utils.unredacted(env_args)
      assert_equal ["-e", "REDIS_PASSWORD=[REDACTED]", "-e", "DB_PASSWORD=[REDACTED]", "-e", "REDIS_URL=\"redis://a/b\"", "-e", "WEB_CONCURRENCY=\"4\""], Pnmx::Utils.redacted(env_args)
    end
  ensure
  ...
  test "env secrets only in role" do
    ...
    @config_with_roles.role(:workers).env_args.tap do |env_args|
      assert_equal ["-e", "DB_PASSWORD=\"secret123\"", "-e", "REDIS_URL=\"redis://a/b\"", "-e", "WEB_CONCURRENCY=\"4\""], Pnmx::Utils.unredacted(env_args)
      assert_equal ["-e", "DB_PASSWORD=[REDACTED]", "-e", "REDIS_URL=\"redis://a/b\"", "-e", "WEB_CONCURRENCY=\"4\""], Pnmx::Utils.redacted(env_args)
    end
  ensure
    ENV["DB_PASSWORD"] = nil
  end
  ...
  test "env secrets only at top level" do
    ...
    @config_with_roles.role(:workers).env_args.tap do |env_args|
      assert_equal ["-e", "REDIS_PASSWORD=\"secret456\"", "-e", "REDIS_URL=\"redis://a/b\"", "-e", "WEB_CONCURRENCY=\"4\""], Pnmx::Utils.unredacted(env_args)
      assert_equal ["-e", "REDIS_PASSWORD=[REDACTED]", "-e", "REDIS_URL=\"redis://a/b\"", "-e", "WEB_CONCURRENCY=\"4\""], Pnmx::Utils.redacted(env_args)
    end
  ensure
    ENV["REDIS_PASSWORD"] = nil
  end
