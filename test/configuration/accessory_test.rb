...
class ConfigurationAccessoryTest < ActiveSupport::TestCase
  ...
  test "env args with secret" do
    ...
    @config.accessory(:mysql).env_args.tap do |env_args|
      assert_equal ["-e", "MYSQL_ROOT_PASSWORD=\"secret123\"", "-e", "MYSQL_ROOT_HOST=\"%\""], Pnmx::Utils.unredacted(env_args)
      assert_equal ["-e", "MYSQL_ROOT_PASSWORD=[REDACTED]", "-e", "MYSQL_ROOT_HOST=\"%\""], Pnmx::Utils.redacted(env_args)
    end
  ensure
