...
class CliAppTest < CliTestCase
  ...
  test "boot uses group strategy when specified" do
    Pnmx::Cli::App.any_instance.stubs(:on).with("1.1.1.1").twice # acquire & release lock
    Pnmx::Cli::App.any_instance.stubs(:on).with([ "1.1.1.1" ]) # tag container

    # Strategy is used when booting the containers
    Pnmx::Cli::App.any_instance.expects(:on).with([ "1.1.1.1" ], in: :groups, limit: 3, wait: 2).with_block_given
  ...
  test "version through main" do
    stdouted { Pnmx::Cli::Main.start(["app", "version", "-c", "test/fixtures/deploy_with_accessories.yml", "--hosts", "1.1.1.1"]) }.tap do |output|
      assert_match "docker ps --filter label=service=app --filter status=running --filter status=restarting --latest --format \"{{.Names}}\" | grep -oE \"\\-[^-]+$\" | cut -c 2-", output
    end
  end
  ...
  private
    def run_command(*command, config: :with_accessories)
      stdouted { Pnmx::Cli::App.start([*command, "-c", "test/fixtures/deploy_#{config}.yml", "--hosts", "1.1.1.1"]) }
    end
