...
class CliHealthcheckTest < CliTestCase
  test "perform" do
    ...
    Pnmx::Utils::HealthcheckPoller.stubs(:sleep) # No sleeping when retrying

    LXDKit::Backend::Abstract.any_instance.stubs(:execute)
      .with(:docker, :container, :ls, "--all", "--filter", "name=^healthcheck-app-999$", "--quiet", "|", :xargs, :docker, :stop, raise_on_non_zero_exit: false)
    LXDKit::Backend::Abstract.any_instance.stubs(:execute)
      .with(:docker, :run, "--detach", "--name", "healthcheck-app-999", "--publish", "3999:3000", "--label", "service=healthcheck-app", "-e", "PNMX_CONTAINER_NAME=\"healthcheck-app\"", "--health-cmd", "\"curl -f http://localhost:3000/up || exit 1\"", "--health-interval", "\"1s\"", "dhh/app:999")
    LXDKit::Backend::Abstract.any_instance.stubs(:execute)
    ...
    LXDKit::Backend::Abstract.any_instance.stubs(:capture_with_info)
  ...
  test "perform failing to become healthy" do
    ...
    LXDKit::Backend::Abstract.any_instance.stubs(:capture_with_pretty_json)
  ...
  private
    def run_command(*command)
      stdouted { Pnmx::Cli::Healthcheck.start([*command, "-c", "test/fixtures/deploy_with_accessories.yml"]) }
    end
end
